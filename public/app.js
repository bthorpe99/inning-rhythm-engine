...
function liveCard(game) {
  ...
  const nextHalfDueUp=(game.nextHalfDueUp||[]).map((hitter,index)=>`<div class="due-hitter next"><small>${index===0?'LEADS OFF':`THEN ${index+1}`}</small><b>${escapeHtml(hitter.name)}</b><span>OPS ${hitter.ops?.toFixed(3)??'—'} · ${hitter.homeRuns??'—'} HR</span></div>`).join('');
