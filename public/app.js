...
let liveState = [];
const pitcherChangeUntil = new Map();
...
async function loadLive() {
  try {
    ...
    for(const game of ordered.filter(item=>item.kind==='LIVE')) {
      const previous=liveState.find(item=>item.gamePk===game.gamePk&&item.kind==='LIVE');
      if(previous?.pitcherId&&game.pitcherId&&previous.pitcherId!==game.pitcherId) pitcherChangeUntil.set(game.gamePk,Date.now()+60000);
      game.pitcherChanged=(pitcherChangeUntil.get(game.gamePk)||0)>Date.now();
    }
    liveState = ordered;
...
function liveCard(game) {
  if (game.kind !== 'LIVE') {
    ...
    const pitchers=`<div class="pregame-pitchers">${pregamePitcher(game.awayPitcherPhoto,game.awayPitcher,game.awayTeam)}<b>VS</b>${pregamePitcher(game.homePitcherPhoto,game.homePitcher,game.homeTeam)}</div>`;
    return `<article class="slate-card upcoming"><span class="slate-kind">${escapeHtml(game.kind)}</span><h3>${escapeHtml(game.awayTeam)} at ${escapeHtml(game.homeTeam)}</h3><strong>${escapeHtml(time || '')}</strong>${pitchers}${pregameLineups(game)}${inningResultStrip(game)}</article>`;
  }
  ...
  const changeBadge=game.pitcherChanged?'<em class="pitcher-change">PITCHER CHANGE — PROJECTION REPRICED</em>':'';
  ...
  const dueUp=(game.dueUp||[]).slice(0,4).map((hitter,index)=>`<div class="due-hitter ${index===0?'current':''}">${hitter.photo?`<img src="${escapeHtml(hitter.photo)}" alt="${escapeHtml(hitter.name)}" loading="lazy">`:''}<div><small>${index===0?'AT BAT':`NEXT ${index}`}</small><b>${escapeHtml(hitter.name)}</b><span>${hitter.homeRuns??'—'} HR</span></div></div>`).join('');
  const nextHalfDueUp=(game.nextHalfDueUp||[]).slice(0,4).map((hitter,index)=>`<div class="due-hitter next">${hitter.photo?`<img src="${escapeHtml(hitter.photo)}" alt="${escapeHtml(hitter.name)}" loading="lazy">`:''}<div><small>${index===0?'LEADS OFF':`NEXT ${index}`}</small><b>${escapeHtml(hitter.name)}</b><span>${hitter.homeRuns??'—'} HR</span></div></div>`).join('');
  const runners=(game.runners||[]).map(runner=>`<div class="base-runner">${runner.photo?`<img src="${escapeHtml(runner.photo)}" alt="${escapeHtml(runner.name)}" loading="lazy">`:''}<span><small>ON ${runner.base===1?'1ST':runner.base===2?'2ND':'3RD'}</small><b>${escapeHtml(runner.name)}</b><em>${runner.homeRuns??'—'} HR</em></span></div>`).join('');
  const runnerPanel=runners?`<div class="base-runners"><small>RUNNERS ON BASE · MLB LIVE</small><div>${runners}</div></div>`:'<div class="base-runners empty"><small>BASES EMPTY</small></div>';
  const futureHalves=(game.futureHalves||[]).map(half=>`<article><header><b>${escapeHtml(half.label)}</b><span>${escapeHtml(half.battingTeam)} BATTING</span></header><div class="future-pitcher">${half.pitcher?.photo?`<img src="${escapeHtml(half.pitcher.photo)}" alt="${escapeHtml(half.pitcher.name)}" loading="lazy">`:''}<span><small>EXPECTED ON MOUND · ${escapeHtml(half.pitchingTeam)}</small><b>${escapeHtml(half.pitcher?.name||'TBD')}</b></span></div><div class="future-batters">${(half.batters||[]).map((hitter,index)=>`<span><small>${index===0?'PROJECTED LEADOFF':`NEXT ${index}`}</small><b>${escapeHtml(hitter.name)}</b><em>${hitter.homeRuns??'—'} HR</em></span>`).join('')}</div></article>`).join('');
  const futurePanel=futureHalves?`<section class="future-halves"><h4>NEXT 3 HALF-INNINGS · PROJECTED</h4>${futureHalves}</section>`:'';
  ...
  return `<article class="slate-card live"><div class="slate-card-top"><span><i class="live-dot"></i>LIVE · INNING ${game.trackedInning} UNDER .5</span><strong class="bet-status ${game.status.toLowerCase()}">${game.status}</strong></div><h3>${escapeHtml(game.awayTeam)} ${game.awayScore} — ${game.homeScore} ${escapeHtml(game.homeTeam)}</h3><p>${escapeHtml(game.half || '')} ${game.inningOrdinal || game.inning} · ${game.outs} outs · ${game.balls}-${game.strikes} · ${bases}</p>${changeBadge}${projection}${inningResultStrip(game)}<div class="slate-pitcher">${game.pitcherPhoto ? `<img src="${escapeHtml(game.pitcherPhoto)}" alt="${escapeHtml(game.pitcher)}">` : ''}<div><small>ON THE MOUND ${verification}</small><strong>${escapeHtml(game.pitcher)} <em class="league-rank">${liveRank}</em></strong><span>${game.pitchCount ?? '—'} pitches · ERA ${game.pitcherEra ?? '—'} · vs ${escapeHtml(game.batter)}</span></div><b>${game.trackedRuns} RUNS</b></div>${runnerPanel}<div class="lineup-pressure"><small>${lineupVerification} · ${escapeHtml((game.half||'CURRENT').toUpperCase())} HALF BATTERS · ${lineupLabel}</small><div>${dueUp}</div></div><div class="lineup-pressure next-half"><small>✓ MLB ${escapeHtml((game.nextHalfLabel||'NEXT HALF').toUpperCase())} BATTERS · ${escapeHtml(game.nextHalfTeam||'')}</small><div>${nextHalfDueUp}</div></div>${futurePanel}<small class="last-play">${escapeHtml(game.lastPlay || 'Waiting for next pitch…')} · Updated ${new Date(game.updatedAt).toLocaleTimeString([], {hour:'numeric',minute:'2-digit',second:'2-digit'})}</small></article>`;
}

function pregamePitcher(photo,name,team) {
  return `<div>${photo?`<img src="${escapeHtml(photo)}" alt="${escapeHtml(name||'Pitcher')}" loading="lazy">`:''}<span><small>${escapeHtml(team||'')}</small><strong>${escapeHtml(name||'TBD')}</strong></span></div>`;
}
