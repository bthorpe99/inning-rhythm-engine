...
function liveCard(game) {
  ...
  const dueUp=(game.dueUp||[]).map((hitter,index)=>`<div class="due-hitter ${index===0?'current':''}">${hitter.photo?`<img src="${escapeHtml(hitter.photo)}" alt="${escapeHtml(hitter.name)}" loading="lazy">`:''}<div><small>${index===0?'AT BAT':`DUE ${index+1}`}</small><b>${escapeHtml(hitter.name)}</b><span>${hitter.homeRuns??'—'} HR</span></div></div>`).join('');
  const nextHalfDueUp=(game.nextHalfDueUp||[]).map((hitter,index)=>`<div class="due-hitter next">${hitter.photo?`<img src="${escapeHtml(hitter.photo)}" alt="${escapeHtml(hitter.name)}" loading="lazy">`:''}<div><small>${index===0?'LEADS OFF':`THEN ${index+1}`}</small><b>${escapeHtml(hitter.name)}</b><span>${hitter.homeRuns??'—'} HR</span></div></div>`).join('');
